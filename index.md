...
$$\log\begin{pmatrix}
\frac{\phi_{i}}{1 - \phi_{i}}
\end{pmatrix} = \beta_{0} + \beta_{1}X_{B} + \beta_{2}X_{C},$$

where $X_{i}$ is a indicator variable that is equal to 1, if
`age_groups` is equal to $i$ and 0 otherwise. In the following example,
we set $\beta_{0} = 2$, $\beta_{1} = - 0.5$ and $\beta_{2} = - 1$ and we
set the arrivals to happen constantly over 4 years.
...
$$\frac{\exp(1.5)}{1 + \exp(1.5)} = 0.818\quad{\mspace{6mu}\text{and}\mspace{6mu}}\quad\frac{\exp(1.5 - 0.8)}{1 + \exp(1.5 - 0.8)} = 0.668$$
